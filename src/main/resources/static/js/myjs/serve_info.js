    ...
    columns: [ //在JS里面定义，field即data-field，title就是每列表头名等等。默认空数组
...

        {
            ...
            formatter: function (value, row) {  //数据转换函数
                var e = '<button button="#" mce_href="#" onclick="alt(\'' + row.serveId + '\')">服务分配</button> ';
                return e;
            }
        }
    ],
...
function updateServe() {
    ...
    var serveId = row.serveId;
    var empCode =$("#allotChan").val();
    var data = {"serveId":serveId,"empCode":empCode,"states":"已分配"};
    $.post("/sys/updateServe.ajax",data,function (sign) {
        if (sign>0){
            //刷新表格
